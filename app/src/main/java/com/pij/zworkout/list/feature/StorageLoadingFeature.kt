 ...
 * @author PierreJean
 */
class StorageLoadingFeature(private val logger: Logger, private val persistence: WorkoutPersistenceUC, private val defaultErrorMessage: String) : AsyncInteraction<Any, Model> {
    ...
    private fun convert(files: List<WorkoutFile>) =
            files.toObservable().map { convert(it) }.toList()
